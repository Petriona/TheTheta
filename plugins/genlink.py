...
@Client.on_message(filters.command('index') & filters.create(allowed))
async def gen_link_batch(bot, message):
    ...
    post = await message.reply_document(f"batchmode_{message.from_user.id}.json", file_name="Batch.json", caption="⚠️Generated for filestore.")
    os.remove(f"batchmode_{message.from_user.id}.json")
    file_id, ref = unpack_new_file_id(post.document.file_id)
    await sts.edit(f"Here is your link\nContains `{og_msg}` files.\n https://t.me/instagramkarobot?start=BATCH-{file_id}")
